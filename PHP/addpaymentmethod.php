...
if (isset($_POST['btnAdd'])) {
	$PMname=$_POST['txtPaymentMethodName'];

    $selectquery="SELECT * FROM paymentmethodtb WHERE PaymentMethodName='$PMname'";
    $runselect=mysqli_query($connect, $selectquery);
    $rowcount=mysqli_num_rows($runselect);
    if ($rowcount>0) {
    	echo "<script>window.alert('Payment Method Already Existed!')</script>";
    	echo "<script>window.location='addpaymentmethod.php'</script>";
    }
    else
    {
        $insert="INSERT INTO paymentmethodtb (PaymentMethodName)
        VALUES('$PMname')";
        $runinsert=mysqli_query($connect,$insert);
        if ($runinsert)
        {
            echo "<script>window.alert('Payment Method Added Successfully!')</script>";
...
?>
 ...
 <head>
    ...
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
 	<title>ADD PAYMENT METHOD | AUTUNNO</title>
 </head>
 <body>
    <header>
        <div class="Dtop">
    ...
    <section>
        <div class="sidebysidediv">
            ...
            <div class="dashboardcontent">
                ...
                <form class="adminadddataform" action="addpaymentmethod.php" method="POST" enctype="multipart/form-data">
                    <label>Payment Method Name</label>
                    <input type="text" name="txtPaymentMethodName" class="textbox" placeholder="Enter Payment Method Name" required>
