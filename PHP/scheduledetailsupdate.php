...
if (isset($_GET['Sid']) && isset($_GET['Hid']))
{
	$Sid=$_GET['Sid'];
    $Hid=$_GET['Hid'];
	$SDupdate="SELECT * FROM scheduledetailstb WHERE ScheduleID = '$Sid' AND HolidayHomeID = '$Hid'";
	$SDquery=mysqli_query($connect, $SDupdate);

	$array=mysqli_fetch_array($SDquery);
	$ScId=$array['ScheduleID'];
	$HHid=$array['HolidayHomeID'];
    $SsDate=$array['ScheduleStartDate'];
    $SeDate=$array['ScheduleEndDate'];
}

if (isset($_POST['btnUpdate'])) 
{
	$ScId=$_POST['txtSId'];
    $HHomeId=$_POST['txtHId'];
    $Sstartdate=$_POST['ssdate'];
    $Senddate=$_POST['sedate'];
	
	$update="UPDATE scheduledetailstb SET ScheduleStartDate='$Sstartdate', ScheduleEndDate='$Senddate' WHERE ScheduleID='$ScId' AND HolidayHomeID='$HHomeId'";
	$updatequery=mysqli_query($connect, $update);

	if ($updatequery){
		echo "<script>window.alert('Schedule Details Updated Successfully!')</script>";
...
<head>
    ...
    <title>UPDATE SCHEDULE DETAILS | AUTUNNO</title>
</head>
<body>
    <header>
        <div class="Dtop">
    ...
    <section>
        <div class="sidebysidediv">
            ...
            <div class="dashboardcontent">
                ...
                <form class="adminadddataform" action="scheduledetailsupdate.php" method="POST">
                    <label>Schedule ID</label>
                    <input type="text" name="txtSId" class="readonlytxt" value="<?php echo $ScId ?>" readonly>
                    
                    <label>Holiday Home ID</label>
                    <input type="text" name="txtHId" class="readonlytxt" value="<?php echo $HHid ?>" readonly>

                    <div class="updatedateinputs">
                        <div>
                            <label>Start Date</label><input type="date" name="ssdate" value="<?php echo $SsDate ?>" required>
                        </div>
                        <div>
                            <label>End Date</label><input type="date" name="sedate" value="<?php echo $SeDate ?>" required>
                        </div>
                    </div>
